// Ported from src/app/data/developmentData.ts (no dependencies)
...
export function generateDevelopmentData(employeeId, readinessScore) {
  const seed = Number.parseInt(employeeId, 10);

  let gapCount;
  let gapScoreDistribution;
  ...
  if (readinessScore >= 81) {
    ...
    if (readinessScore < 40) gapCount = 6;
    else if (readinessScore < 50) gapCount = 5;
    else gapCount = 4;
  ...
  const competencyDetails = STANDARD_COMPETENCIES.map((name, index) => {
    const isGap = gapIndices.includes(index);
    if (isGap) {
      const gapIndexInDistribution = gapIndices.indexOf(index);
      return { name, score: gapScoreDistribution[gapIndexInDistribution] };
    }

    let score;
    if (readinessScore >= 90) score = 5;
    else if (readinessScore >= 81) score = seededRandom(seed * (20 + index)) > 0.3 ? 5 : 4;
    else if (readinessScore >= 66) score = seededRandom(seed * (30 + index)) > 0.5 ? 4 : 3;
    else score = seededRandom(seed * (40 + index)) > 0.7 ? 4 : 3;

    return { name, score };
  });

  const idpRecommendations = [];
  gapIndices.forEach((index) => {
    const competencyName = STANDARD_COMPETENCIES[index];
    const template = IDP_TEMPLATES[competencyName];
    if (template) idpRecommendations.push(template);
  });

  if (idpRecommendations.length === 0) {
    idpRecommendations.push({
  ...
  return { competencyDetails, idpRecommendations };
}

export function calculateCompetencyScore(competencyDetails) {
